...
describe("filterDataByEmployer()", () => {
  test("should throw an error if people is empty", () => {
    ...
    }).toThrow("The `people` array is empty.");
  });
...
describe("getPersonByName()", () => {
  test("should throw an error if people is empty", () => {
    ...
    const last = "Fail";
    expect(() => {
      getPersonByName(people, first, last);
    }).toThrow("The `people` array is empty.");
  });
  ...
  test("should throw an error if person is not found", () => {
    ...
    expect(() => {
      getPersonByName(people, first, last);
    }).toThrow("Person with given name could not be found.");
  });
  ...
  test("should return a matching person if name is found", () => {
    ...
    const expected = {
      ...
      first_name: "Isidora",
      last_name: "Reynard",
      email: "ireynardb9@businessweek.com",
      gender: "Genderqueer",
      ip_address: "176.201.151.140",
      credit_card: { number: "675950198230116676", type: "switch" },
      username: "ireynardb9",
      employer: "Gabtune",
    };
